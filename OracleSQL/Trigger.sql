...
FOR EACH ROW
declare maxlogid number; pkchanged char(1);
begin  Insert into Rep_LogTable values ( S_Rep_LogTable.nextVal , 'I_ITM_STOREITEM');
Select max(Rep_cid) into maxlogid from Rep_LogTable;
if( :oldRow.ID!= :newRow.ID ) THEN  pkChanged := 'Y';
END IF;
Insert Into R_SD_I_ITM_STOREITEM ( 
    Rep_sync_id, Rep_common_id, 
    Rep_operationType, Rep_status, 
    ID , 
    ITEMCODE , 
    COUNTERCODE , 
    PROMOTIONMARK , 
