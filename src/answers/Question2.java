...
public class Question2
{
	public static class Q1Map extends Mapper<LongWritable,Text,Text,Text>
	{
		...
		public void map(LongWritable key,Text value,Context context) throws IOException, InterruptedException
		{
			...
			{
				if(Integer.parseInt(kv[0])>0&&Integer.parseInt(kv[0])<10&&!kv[1].toString().equals("\\N"))
				{
					context.write(new Text(kv[1].trim()+","+devMap.get(kv[0]).trim()), new Text((kv[2].trim())));
				}
			}
		}
	}
	...
	public static class Q1Reduce extends Reducer<Text,Text,Text,Text>
	{
		public void reduce(Text key, Iterable<Text> values, Context context) throws IOException, InterruptedException
		{
			...
			if(count==0)
				...
				context.write(key, new Text(sumValue.divide(new BigDecimal(count),8,4).toString()));
		}
	}
	...
	public static class Q1Sort extends WritableComparator
	{
		...
		{
			String []sa=((Text)a).toString().split(",");
			String []sb=((Text)b).toString().split(",");
			int r=sb[0].compareTo(sa[0]);
			if(r!=0)
				return r;
