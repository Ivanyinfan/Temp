...
sys_page_alloc(envid_t envid, void *va, int perm)
{
	...
	if(page==NULL)
		return -E_NO_MEM;
	r=page_insert(env_store->env_pgdir,page,va,perm);
	if(r)
		return r;
	return 0;
}
	     ...
	     envid_t dstenvid, void *dstva, int perm)
{
	...
	int r=envid2env(srcenvid,&srcenv_store,1);
	if(r)
		return r;
	r=envid2env(dstenvid,&dstenv_store,1);
	if(r)
		return r;
	pte_t *pte;
	struct Page *page=page_lookup(srcenv_store->env_pgdir,srcva,&pte);
	if(page==NULL)
		return -E_INVAL;
	if((perm&PTE_W)&&(*pte&PTE_W)==0)
		return -E_INVAL;
	r=page_insert(dstenv_store->env_pgdir,page,dstva,perm);
	if(r)
		return r;
	return 0;
}
...
{
	// LAB 4: Your code here.
	//cprintf("kern/syscall.c [sys_ipc_try_send] envid=%d,value=%d,srcva=%p,perm=%d\n",envid,value,srcva,perm);
	if((uintptr_t)srcva<UTOP&&(uintptr_t)srcva%PGSIZE)
		return -E_INVAL;
	perm=PTE_P|PTE_U;
	if((uintptr_t)srcva<UTOP&&(perm&~PTE_SYSCALL))
		return -E_INVAL;
	//cprintf("kern/syscall.c [sys_ipc_try_send] ...\n");
	struct Env *e;
	...
	if((uintptr_t)srcva<UTOP)
	{
		...
		if(page==NULL||(perm&PTE_W&&(*pte&PTE_W)==0))
			return -E_INVAL;
		cprintf("kern/syscall.c [sys_ipc_try_send] ...\n");
...
{
	lock_kernel();
	int r=syscall(syscallno,a1,a2,a3,a4,a5);
	unlock_kernel();
	return r;
}
